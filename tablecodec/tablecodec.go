...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	if !key.HasPrefix(tablePrefix) {
		err = errInvalidKey.GenWithStack("table prefix not found and invalid key - %q", k)
		return
	}
	...
	if !key.HasPrefix(recordPrefixSep) {
		err = errInvalidKey.GenWithStack("record prefix not found and invalid key - %q", k)
		return
	}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	if !key.HasPrefix(indexPrefixSep) {
		err = errInvalidKey.GenWithStack("index prefix not found and invalid key - %q", k)
		return
	}
